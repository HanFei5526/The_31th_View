...
- `chapter2` / `chapter3` / `finale`：已注册真实场景；测试阶段菜单章节全开，便于直接跳转验证。
...
- 模型：`deepseek-chat`
- 接口：前端请求 `/api/chat`，由 `Web/server` 代理转发到 DeepSeek
- API Key：通过 `Web/server/.env` 中的 `DEEPSEEK_API_KEY` 配置
- 前端直连：已废弃，浏览器端不持有真实 API Key
...
| `generateFinalNote` / `generateReport` | 终章结局后生成沈念结案笔记（旧接口保留兼容） | 单轮 |
...
- AI 只能基于已解锁的知识片段回答
- 第一章 `蘅` 字解释在获得断簪后开放，低处观察说明在缀云峰低视角探索后开放
- 第三章按散落草图、渗字、朱砂线、墙面草图、蹲下验证、信件、匾额追认逐步开放知识
- 终章只允许回顾记录，不开放 AI 代答四问或推荐结局
- 终章三结局正文固定；AI 只在结局卡之后可选生成“修复记录终页”，不写正式档案或主线结局
- 系统提示只做引导或确认，不直接解锁 AI 知识；知识边界跟随线索记录、物件获得和关键进度标记
- 普通探索时笔记本默认收起，轻量讨论/综合研讨自动展开，讨论结束或切入下一探索段前自动收起
- 章节入口补齐前序资料只作为旧存档/跳章节兜底，不作为普通流程的知识获得方式
...
- 存档使用 `localStorage`，键名为 `sanyijing-save`，不上传服务器，不跨浏览器或设备同步
- 自动保存内容包括：当前章节、当前场景、世界状态、物件匣、进度标记、笔记本记录、按章节隔离的笔记本对话
- 返回菜单不清除进度；刷新页面或关闭后重开仍可继续
- “继续游戏”读取最近一次本地存档，若旧存档记录在菜单/着陆页，则回退到最近已解锁章节
- “重新开始”会清空本地存档和当前运行时状态，从序章重新开始
- 进入非菜单/非着陆页场景时会自动保存一次，避免刚进入新章节后刷新回到旧场景
- 菜单章节入口测试阶段保持全开；上线前需切回按完成标记解锁：序章默认开放，第一章在序章后开放，第二章在第一章后开放，第三章在第二章后开放，终章在第三章后开放
- 关键物件支持从进度标记兜底补齐：修复笔记本、断簪、残砚、草图拓片、王蘅的信
...
4. **AI 知识边界补全**（已完成）：第一章解锁时机收紧，第三章知识片段补齐，终章改为记录回顾边界
5. **系统提示与笔记本节奏校准**（已完成）：系统提示不直接解锁知识，探索收起笔记本，讨论自动展开并在结束后收起
6. **完整流程与本地存档校准**（已完成）：非菜单场景入口自动保存，关键物件可由进度兜底补齐；菜单章节测试阶段全开，上线前切回按进度解锁
7. **终章结案笔记**（已完成）：三结局正文保持固定，结局卡后可选生成沈念个人“修复记录终页”
8. **第三十一景资产统一与视角转场**（计划中）：统一第31景美人靠栏杆构图并移除“檀雷堂”假题字；四问锁定视角后，底图由大地图线稿转为同构第31景线稿，支持四象限渐进填色复原。

上线前待处理：

- 菜单章节入口从“测试阶段全开”切回“按完成标记逐章解锁”。
- 清理未使用资产（如 `finale-view31-high.png`）。
...
- 菜单、首页、过场类装饰背景可使用 `cover` 铺满视口，但需为主要视觉焦点设置稳定的 `background-position`。
- 解谜、线索、古画观察类画面不得只依赖会裁切的 `cover` 背景承载关键信息，需使用安全区域、查看器或固定画幅。
- 若要求全屏 / 非全屏显示范围完全一致，应改用固定比例舞台并单独评估布局影响。
