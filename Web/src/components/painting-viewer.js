...
const CLUE_DEFS = {
  ...
  clue_text:   { x: 6.5,  y: 75.5, r: 8,  title: '"……所见"残字' },
...
// ── 工具反馈文案 ─────────────────────────────────────
const TOOL_FEEDBACK = {
  magnifier: '放大镜已就位。装裱接缝处有重叠痕迹，边框似乎压住了旧题签的一角。（检测结果已同步至右侧【修复笔记本】的【记录】页，可以前往查看或在【对话】页发起研讨）',
  fiber:     '纸质分析完成。背纸与其他三十页不完全一致，此页曾经重装；画心本身较稳定。（检测结果已同步至右侧【修复笔记本】的【记录】页，可以前往查看或在【对话】页发起研讨）',
  sidelight: '侧光照射完成。装裱边下方隐约显出旧字残痕和一条极淡的细线。（检测结果已同步至右侧【修复笔记本】的【记录】页，可以前往查看或在【对话】页发起研讨）',
};
...
const TOOL_FILTERS = {
  ...
  sidelight:  { transform: 'translate(43.5%, -25.5%) scale(1.25)',   filter: 'brightness(0.70) contrast(1.65) sepia(0.35) saturate(0.8)' },
};
        ...
        if (toolId === 'magnifier') {
          ...
          if (this._explorable) {
            ...
            if (this._zoomLevel > 1.0 || this._panX !== 0 || this._panY !== 0) {
              ...
              // 探索状态下，未做滚轮放大时回归原位（scale 1），不强制聚焦右上角
              transform = '';
            }
          } else {
            // 基础检查阶段（未解锁探索时），点击放大镜使用预设的右上角缩放聚焦效果
