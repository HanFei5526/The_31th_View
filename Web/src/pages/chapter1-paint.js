...
export default class Chapter1PaintScene {
  ...
  async _askNotebook(text, meta = {}) {
    ...
    try {
      const reply = await this.engine.aiService.queryNotebook(text, {
        ...
        chapterScene: 'chapter1-paint',
        hairpinIdentified: this._hairpinIdentified,
  ...
  _buildLanxueScene() {
    ...
    const plaqueSpot = this._createHotspot(50, 22, 8, async () => {
      ...
      await this.narrationBar.playLine('沈念', '这座厅后面还有路。石径延伸过去，像是有什么在更深处等着。', { portrait: '/images/shennian_1.png' });
  ...
  _buildZhuiyunScene() {
    ...
    this._zhuiyunArrow = this._createNavArrow(75, 65, 10, async () => {
      if (this._isNarrating) return;
      this._isNarrating = true;
      await this.narrationBar.playLine('沈念', '前面有水声。像是快到什么水边的建筑了。', { portrait: '/images/shennian_1.png' });
      this.narrationBar.dismiss();
      this._isNarrating = false;
  ...
  _buildFurongScene() {
    ...
    const onWaterlineInteract = async () => {
      ...
      if (!this._isFlipped) {
        // 翻转前叙事
        this._isNarrating = true;
        await this.narrationBar.playLine('沈念', '如果倒影才是真的那一面呢——', { portrait: '/images/shennian_2.png' });
        this.narrationBar.dismiss();
        this._isNarrating = false;
  ...
  async _startLightDiscussion() {
    ...
    this.notebook.showNPCMessage('周老师批注："蘅"，杜衡。是一种香草，古人也用来比喻品性高洁的女子。刻在簪身背面，不是正面——如果是工匠标记，通常会在簪头或底座。这更像是物主自己留给自己的。那么，是谁把自己的名字藏在了一支断簪上？');
