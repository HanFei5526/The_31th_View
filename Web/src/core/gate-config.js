...
export const GATE_CONFIG = {
  ...
  gate_prologue_synthesis: {
    ...
    concepts: [
      {
        ...
        patterns: [
          ...
          /(?:装裱|重装|整理|配边|归档).{0,12}(?:遮蔽|遮盖|遮住|压住|裁去|处理|排除)/,
          /(?:遮蔽|遮盖|遮住|压住|裁去|排除).{0,12}(?:来源|说明|边注|题签|残字|辅助线|痕迹)/,
        ],
    ...
    passResponse: '对。现在能确认的不是画心丢失，也不是画被重画，而是这页画的来源说明被后来的整理和装裱处理遮蔽了。三处痕迹共同指向这一点。',
    hintPool: [
      '（翻到一页批注）旧题签、残字、底层细线——先不要分开看。它们都不在画心主体里。',
      '（夹着一张便签）如果只是保存不善，痕迹通常会散乱；这三处却都贴着边缘、装裱层或底层。',
      '（此处有一段红笔批注）题签、边注和底层细线都像说明性材料的残留。先判断它们是自然留下的，还是后来处理留下的。',
    ],
    affirmPool: [
      '（页边有一行小字）方向接近。还需要把"自然破损"和"后来处理"区分清楚。',
      '（页边标注）关联性已经建立。继续看三处痕迹共同处在什么位置。',
      '（页边有一行小字）不像自然损坏——这个判断很重要，但还要说明处理方式。',
    ],
    correctionPool: [
      '（翻到另一页批注）不是画心失踪，也不是新画替换。画还在，问题出在画心之外的说明性痕迹。',
      '（此处有一段红笔批注）暂时不要找具体姓名。现有证据只能支持对痕迹处理方式的判断。',
      '（页角折过，翻开看）如果坚持自然破损，就解释不了三处痕迹为何都集中在边缘、装裱层和底层。',
    ],
    quickThoughts: [
      '旧题签、残字和细线是不是都在画心之外？',
      '这三处痕迹有没有可能原本在说明同一件事？',
      '如果不是自然破损，装裱或整理时可能发生了什么？',
    ],
    fallbackQuickThoughts: [
      '这些痕迹更像自然破损，还是后来的处理结果？',
      '装裱和整理会不会把画心之外的说明压住？',
      '先不找具体姓名，只判断这些痕迹是不是被人为遮住了。',
    ],
  },
};
...
export function analyzePlayerInputAccumulated(playerInput, accumulated = []) {
  ...
  if (_blocksSynthesisAccumulation(input)) {
    return {
      matchedConcepts,
